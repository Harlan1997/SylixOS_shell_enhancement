...
#define __TTINY_SHELL_CMD_ISEND(pcCmd)      (*(pcCmd) == PX_EOS)
     ...
     *  查找关键字.
     */
    ULONG ret = __tshellKeywordFind(cKeyword, &pskwNode);
    if (ERROR_NONE != ret) {       /*  查找关键字                  */
        if(ret == ERROR_TSHELL_INCOMPLETE)
        {
            return (-ERROR_TSHELL_INCOMPLETE);
        }
...
ULONG  __tshellKeywordFind (CPCHAR  pcKeyword, __PTSHELL_KEYWORD   *ppskwNode)
{
    ...
    for (;
        ...
        if (lib_strcmp(tmpKw, pskwNode->SK_pcKeyword) == 0) {       /*  关键字相同                  */
            break;
        }
    }
    if (plineHash == LW_NULL) {
        CPCHAR allKeyWords[150] = {"aborts", "affinity", "buss", "clear", "color", "cpuus",
                            "devs", "drvlics", "drvs", "env", "echo", "eventset", "exec",
                            "exit", "free", "help", "hostname", "ints", "kill", "login",
                            "loglevel", "mems", "part", "pcis", "ps", "reboot", "renice",
                            "restart", "sem", "shell", "shstack", "shutdown", "sigqueue",
                            "sleep", "sprio", "ss", "top", "ts", "tty", "vardel", "varload",
                            "vars", "varsave", "virtuals", "which", "who", "zone", "cat",
                            "cd", "ch", "chmod", "close", "cmp", "cp", "df", "dosfslabel",
                            "dsize", "fatugid", "fdentrys", "fdisk", "fils", "gzip", "ll",
                            "ln", "logfileadd", "logfiles", "ls", "mkdir", "mkfifo", "mkfs",
                            "mmaps", "mount", "msgq", "mv", "open", "pwd", "rm", "rmdir",
                            "showmount", "shfile", "sync", "tmpname", "touch", "umount", 
                            "untar", "vi", "yaffscmd", "zlib", "gadd", "gdel", "group",
                            "pmod", "uadd", "udel", "umod", "user", "aodvs", "arp", "ftpdath",
                            "hosttable", "ifconfig", "ifdown", "ifrouter", "ifup", "ipv6",
                            "nat", "natalias", "natmap", "nats", "nbname", "netstat", "npfattach",
                            "npfruleadd", "npfruledel", "npfs", "ping", "ping6", "route", "tftp",
                            "tftpdpath", "vlan", "vpnclose", "vpnopen", "date", "hwclock", "times",
                            "tzsync", "debug", "dlconfig", "leakchk", "leakchkstart", "leakchkstop",
                            "lsmod", "modulegcov", "modulereg", "modules", "modulestat", "moduleunreg",
                            "args", "crypt", "perfrefresh", "perfs", "perfstart", "perfstop", 
                            "smodemr", "xmodems"
        };
        INT l;
        for (l = 0; l < 150; l++) {      
            CPCHAR currKw = allKeyWords[l];                     
            CPCHAR existKw = allKeyWords[l];
            tmpKw = pcKeyword;
            INT len = lib_strlen(tmpKw);
            INT i = 0;
            INT j = 0;
            while(*existKw == *tmpKw)
            {
